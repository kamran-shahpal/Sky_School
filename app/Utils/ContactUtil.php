...
class ContactUtil extends Util
{
    ...
    public function getContactQuery($business_id, $type, $contact_ids = [])
    {
        ...
        if ($type == 'supplier') {
            $query->addSelect([
                DB::raw("COALESCE(SUM(IF(t.type = 'purchase' AND t.status = 'final', final_total, 0)),0)-COALESCE(SUM(IF(t.type = 'purchase' AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)),0)+COALESCE(SUM(IF(t.type = 'opening_balance', final_total, 0)),0) -COALESCE(SUM(IF(t.type = 'opening_balance', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)),0) as for_ordering_total_due")
            ]);
        }
        else{
            $query->addSelect([
                DB::raw("COALESCE(SUM(IF(t.type = 'sell' AND t.status = 'final', final_total, 0)),0)-COALESCE(SUM(IF(t.type = 'sell' AND t.status = 'final', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)),0)+COALESCE(SUM(IF(t.type = 'opening_balance', final_total, 0)),0) -COALESCE(SUM(IF(t.type = 'opening_balance', (SELECT SUM(IF(is_return = 1,-1*amount,amount)) FROM transaction_payments WHERE transaction_payments.transaction_id=t.id), 0)),0) as for_ordering_total_due")
            ]);
        }
