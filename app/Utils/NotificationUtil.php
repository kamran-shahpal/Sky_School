...
class NotificationUtil extends Util
{
     ...
     * @return void
     */
    public function autoSendNotificationPayAtOnce($business_id, $notification_type, $request=null,$contact_id=null)
    {
        $notification_template = NotificationTemplate::where('business_id', $business_id)
                ->where('template_for', $notification_type)
                ->first();
        $contact = Contact::where('business_id', $business_id)->find($request->contact_id);
        $business = Business::findOrFail($business_id);
        $data['email_settings'] = $business->email_settings;
        $data['sms_settings'] = $business->sms_settings;
        $whatsapp_link = '';
        if (!empty($notification_template)) {
            if (!empty($notification_template->auto_send) || !empty($notification_template->auto_send_sms) || !empty($notification_template->auto_send_wa_notif) ) {
                ...
                $tag_replaced_data = $this->replaceDueTags($business_id, $orig_data,$contact,$request->amount);
     ...
     * @return array
     */
    public function replaceDueTags($business_id, $data, $contact,$paid_amount=null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (strpos($value, '{cumulative_due_amount}') !== false) {
                $due = $this->getContactDue($contact->id);
